...
const sellMenuSwitch = new TelegrafInlineMenu((ctx) => checkWeekDay(ctx, today))
...
menuSwitch
  .submenu('🍀 Cadastrar Turnips', 'regturnipssw', menuTurnipsSwitch)
menuSwitch
  .simpleButton('📜 Listar Friend Code', 'listfcsw', {
    doFunc: async (ctx) => await listFC({ ctx, flag: 'Switch' })
  })
menuSwitch
  .simpleButton('📈 Listar Turnips', 'listturnipssw', {
...
sellMenuSwitch
  .question('Manhã', 'morningsw', {
    ...
    setFunc: async function swMorning (ctx) {
      if (ctx.message.text.match(/^\d{2,3}$/)) {
        return await registerTurnipsSell({ ctx: ctx, today: today, flag: 'Switch', flagTime: 'morning' })
      }
  ...
  .question('Tarde', 'noonsw', {
    ...
    setFunc: async (ctx) => {
      if (ctx.message.text.match(/^\d{2,3}$/)) {
        return await registerTurnipsSell({ ctx: ctx, today: today, flag: 'Switch', flagTime: 'noon' })
      }
