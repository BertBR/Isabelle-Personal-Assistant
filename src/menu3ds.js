...
menu3ds
  .submenu('🍀 Cadastrar Turnips', 'regturnips3ds', menuTurnips3ds)
menu3ds
  .simpleButton('📜 Listar Friend Code', 'listfc3ds', {
    doFunc: async (ctx) => await listFC({ ctx, flag: '3ds' })
  })

menu3ds
  .simpleButton('📈 Listar Turnips', 'listturnips3ds', {
...
sellMenu
  .question('Manhã', 'morning3ds', {
    ...
    setFunc: async (ctx) => {
      if (ctx.message.text.match(/^\d{2,3}$/)) {
        return await registerTurnipsSell({ ctx: ctx, today: today, flag: '3ds', flagTime: 'morning' })
      }
  ...
  .question('Tarde', 'noon3ds', {
    ...
    setFunc: async (ctx) => {
      if (ctx.message.text.match(/^\d{2,3}$/)) {
        return await registerTurnipsSell({ ctx: ctx, today: today, flag: '3ds', flagTime: 'noon' })
      }
