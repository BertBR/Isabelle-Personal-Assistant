...
function checkWeekDay (ctx, today) {
  if (today === 0) {
    ctx.reply('Função disponível somente de Segunda a Sábado!\nDigite /start para voltar!')
    return
  }
...
async function createUserFC ({ ctx, flag }) {
  ...
  if (user === undefined && flag === '3ds') {
    await connection('3ds_users')
      .insert({
        id: userId,
        username: username,
        name: userFirstName,
  ...
  if (user === undefined && flag === 'Switch') {
    await connection('switch_users')
      .insert({
        id: userId,
        username: username,
        name: userFirstName,
...
async function listFC ({ ctx, flag }) {
  if (flag === '3ds') {
    list3ds = await connection('3ds_users')
      .select('name', 'username', 'fc', 'fruit_type')
    ...
    return ctx.replyWithMarkdown(`*Aqui está a lista de FC (3DS):*\n\n${list3ds.toString().replace(/,/g, '')}`, { disable_web_page_preview: true })
  }

  listSw = await connection('switch_users')
    .select('name', 'username', 'fc', 'fruit_type')
  ...
  return ctx.replyWithMarkdown(`*Aqui está a lista de FC (SWITCH):*\n\n${listSw.toString().replace(/,/g, '')}`, { disable_web_page_preview: true })
}
...
async function registerTurnipsBuy ({ ctx, flag }) {
  ...
  if (user === undefined) {
    return ctx.reply('Você ainda não cadastrou seu FC. Por favor, cadastre-se antes de inserir os preços dos Turnips')
  }
...
async function registerTurnipsSell ({ ctx, today, flag, flagTime }) {
  ...
  const turnip = await checkTurnipSell({ today: today, userId: userId, flag: flag })
...
async function listTurnips ({ ctx, flag, today }) {
  ...
  if (today === 0) {
    ...
    return ctx.replyWithMarkdown(finalList.toString().replace(/,/g, ''), { disable_web_page_preview: true })
  }
...
async function registerFruit ({ ctx, flag, key }) {
  ...
  if (user === undefined) {
    return ctx.reply('Você ainda não cadastrou seu FC. Por favor, cadastre-se antes de informar sua fruta nativa')
  }
